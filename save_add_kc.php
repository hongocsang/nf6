<?php
    ...
    $sql = "insert into khongcap(`nam`, `nguoi_luu`, `ghi_chu`)
    values('[".$nam."]','".$nguoi_luu."','".$ghi_chu."')";
    ...
    {   
        if (mysqli_query($conn, $sql))
        {
            ...
            $sqlKe = "update ke set `bang` ='khongcap',`id_bang` = ".$id_bang."
            WHERE `vt_ke` = ".$vt_ke." and `vt_cot` = ".$vt_cot." and `vt_hang` = ".$vt_hang."";

            if (mysqli_query($conn, $sqlKe))
            {
                echo "Thêm ct thành công";  
                echo "<script>alert('thêm thành công'); window.location.href='add_kc.php';</script>";
            }else
            {   
    ...
    if (mysqli_query($conn, $sql))
    {
        ...
        $sqlKe = "insert into ke(`vt_ke`, `vt_cot`, `vt_hang`, `bang`, `id_bang`) 
            values(".$vt_ke.",".$vt_cot.",".$vt_hang.",'khongcap',".$id_bang.")";

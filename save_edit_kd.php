<?php
    ...
    $sql_kd = "select * from khongdat where id =".$id_bang."";
    $result_kd = mysqli_query($conn, $sql_kd);
    $data_kd = mysqli_fetch_assoc($result_kd);

    if($data_kd){
        if($data_kd["nam"] != $nam)
        {
            $sql = "update khongdat set nam=".$nam." where id=".$id_bang."";
            mysqli_query($conn, $sql);
        }
        if($data_kd["ktb"] != $ktb)
        {
            $sql = "update khongdat set ktb=".$ktb." where id=".$id_bang."";
            mysqli_query($conn, $sql);
        }
        if($len_gc > 0)
        {   
            $result_ghichu = $data_kd["ghi_chu"]. "<br />" .$ghi_chu;
            $sql = "update khongdat set ghi_chu='".$result_ghichu."' where id=".$id_bang."";
            mysqli_query($conn, $sql);
        }
    }else
    {

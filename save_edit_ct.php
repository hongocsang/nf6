<?php
    ...
    $sql_ct = "select * from chungthu where id =".$id_bang."";
    $result_ct = mysqli_query($conn, $sql_ct);
    $data_ct = mysqli_fetch_assoc($result_ct);

    if($data_ct){
        if($data_ct["nam"] != $nam)
        {
            $sql = "update chungthu set nam=".$nam." where id=".$id_bang."";
            mysqli_query($conn, $sql);
        }
        if($data_ct["thung_so"] != $thungso)
        {
            $sql = "update chungthu set thung_so=".$thungso." where id=".$id_bang."";
            mysqli_query($conn, $sql);
        }
        if($data_ct["yk_start"] != $yk_start)
        {
            $sql = "update chungthu set yk_start=".$yk_start." where id=".$id_bang."";
            mysqli_query($conn, $sql);
        }
        if($data_ct["yk_end"] != $yk_end)
        {
            $sql = "update chungthu set yk_end=".$yk_end." where id=".$id_bang."";
            mysqli_query($conn, $sql);
        }
        if($len_gc > 0)
        {   
            $result_ghichu = $data_ct["ghi_chu"]. "<br />" .$ghi_chu;
            $sql = "update chungthu set ghi_chu='".$result_ghichu."' where id=".$id_bang."";
            mysqli_query($conn, $sql);
        }
    }else
    {

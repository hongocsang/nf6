<?php
    ...
    values(".$nam.",'[".$thungso."]',".$yk_start.",".$yk_end.",1,'".$nguoi_luu."','".$ghi_chu."')";
    ...
    {   
        if (mysqli_query($conn, $sql))
        {
            ...
            if (mysqli_query($conn, $sqlKe))
            {
                ...
                echo "<script>alert('thêm thành công'); window.location.href='add_ct_neu.php';</script>";
            }else
            {   
    ...
    if (mysqli_query($conn, $sql))
    {
        ...
        echo "Thêm ct thành công";  
        echo "<script>alert('thêm chứng thư thành công'); window.location.href='add_ct_neu.php';</script>";

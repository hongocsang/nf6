...
<head>
    ...
    <title>Thêm giấy chống giả hỏng</title>
    <link rel="stylesheet" href="add.css">
</head>
<body>
    <h1>THÊM GIẤY CHỐNG GIẢ HỎNG</h1>
    <a href="index.php">Trở về trang chủ</a> <br/> <br/>
    <table border="1" cellspacing="0" id="customers">
        <?php
            echo "<form action='save_add_cgh.php' method='post' enctype='multipart/form-data'>";
            if(isset($_POST['add']))
            {
                $data = $_POST['add'];
                $vt = explode('_', $data);
            ...
            echo "<tr><td>Năm:</td> <td><input type='int' name='nam' maxlength='4'></td></tr>";
            echo "<tr><td>Thùng số:</td> <td><input type='text' name='thung'></td></tr>";
            echo "<tr><td>Có quốc huy:</td> <td><input type='number' name='cqh'></td></tr>";
            echo "<tr><td>Không có quốc huy:</td> <td><input type='number' name='kcqh'></td></tr>";
